...
const pillInfo =  (itemName, callback) => {
    const url = 'http://apis.data.go.kr/1470000/DURPrdlstInfoService/getCpctyAtentInfoList?';

    const ServiceKey = serviceKey0.publicPortalKey;

    var queryParams = encodeURIComponent('serviceKey') + '=' + ServiceKey;
    queryParams += '&' + encodeURIComponent('typeName') + '=' + encodeURIComponent('용량주의');
    ...
    request(fullurl, (error, body) => { // body는 url의 데이터. 여기서는 XML 형식으로 제공되었다.
        ...
        const pill = JSON.parse(hello); // 파싱(parse)한 데이터는 한줄로 쭈우우우욱 나열된다.
        callback(null, { pill }) // undefined는 에러
    });
};

module.exports = pillInfo;